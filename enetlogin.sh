...
do
	...
	if [ $ret_code -ne 200 ] ; then
		...
		execution=$(curl --silent --cookie-jar /tmp/cascookies -H "User-Agent: -H "User-Agent: Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:71.0) Gecko/20100101 Firefox/71.0"" -L "$eneturl" | grep -P -o '(?<=name="execution" value=").*(?="/><input type="hidden" name="_eventId)')
